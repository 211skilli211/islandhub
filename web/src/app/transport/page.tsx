    ...
    return (
        <main className="min-h-screen bg-black">
                ...
                <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-20 pb-24 text-center">
                    <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }}>
                        ...
                        <p className="text-gray-400 text-lg sm:text-xl max-w-2xl mx-auto mb-10 leading-relaxed">
                            ...
                            <span className="text-gray-500">Tap a ride. Get moving.</span>
                        </p>
            ...
            <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 -mt-6 relative z-10">
                <div className="bg-gray-900/80 backdrop-blur-xl border border-white/5 rounded-2xl p-5 shadow-2xl">
                    <div className="flex flex-col md:flex-row gap-4">
                        <div className="flex-1 relative">
                            <input type="text" placeholder="Where to?" value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="w-full pl-12 pr-4 py-3.5 rounded-xl bg-black/60 border border-white/10 text-white placeholder-gray-500 focus:border-yellow-400/50 focus:ring-2 focus:ring-yellow-400/20 transition-all outline-none" />
            ...
            <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                    {TRANSPORT_TYPES.map(type => (
                        <button key={type.id} onClick={() => setSelectedType(type.id)} className={`group relative p-6 rounded-2xl text-center transition-all duration-300 overflow-hidden ${selectedType === type.id ? 'bg-yellow-400 text-black shadow-xl shadow-yellow-400/20 scale-[1.02]' : 'bg-gray-900/60 text-white border border-white/5 hover:border-yellow-400/30 hover:bg-gray-900/80'}`}>
                            {/* Hover glow */}
                            ...
                            <div className="relative">
                                ...
                                <p className={`text-xs mt-1.5 ${selectedType === type.id ? 'text-black/60' : 'text-gray-500'}`}>{type.desc}</p>
                            </div>
                        </button>
                    ))}
                </div>
            ...
            <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pb-16">
                <div className="flex items-center justify-between mb-8">
                    ...
                    {!loading && filteredTransport.length > 0 && (
                        <span className="text-sm text-gray-500">{filteredTransport.length} available</span>
                    )}
                </div>
                {loading ? (
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                        {[1,2,3].map(i => (<div key={i} className="bg-gray-900 rounded-2xl h-72 animate-pulse border border-white/5" />))}
                    </div>
                            ...
                            {filteredTransport.map((store, index) => (
                                <motion.div key={store.id || store.store_id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: index * 0.05 }}>
                                    <Link href={`/store/${store.slug}`}>
                                        <div className="bg-gray-900/60 border border-white/5 rounded-2xl overflow-hidden group cursor-pointer h-full hover:border-yellow-400/20 transition-all duration-300 hover:shadow-lg hover:shadow-yellow-400/5">
                                            ...
                                            <div className="p-5">
                                                <div className="flex items-center gap-3 mb-3">
                                                    ...
                                                    <div className="flex-1 min-w-0">
                                                        ...
                                                        {store.rating && (<div className="flex items-center gap-1"><span className="text-yellow-400 text-sm">★</span><span className="text-sm text-gray-400">{store.rating}</span></div>)}
                                                    </div>
                                                </div>
                                                <p className="text-gray-500 text-sm line-clamp-2 mb-4">{store.description}</p>
                                                <div className="flex items-center justify-between">
                                                    <span className="text-xs text-gray-600 uppercase tracking-wider">{store.subtype || store.category}</span>
                ...
                <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pb-16">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                        {filteredOther.map((store, index) => (
                            <motion.div key={store.id || store.store_id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: index * 0.05 }}>
                                <Link href={`/store/${store.slug}`}>
                                    <div className="bg-gray-900/40 border border-white/5 rounded-2xl overflow-hidden group cursor-pointer h-full hover:border-white/10 transition-all duration-300">
