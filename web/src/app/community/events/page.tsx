            ...
            >
                {step === 'success' ? (
                    ...
                    <div className="p-8 text-center">
                        ...
                        <h2 className="text-2xl font-black text-primary mb-2">Tickets Purchased! 🎉</h2>
                        <p className="text-sm text-tertiary mb-2">You're going to {event.title}</p>
                        <p className="text-xs text-tertiary mb-6">{totalTickets} ticket{totalTickets !== 1 ? 's' : ''} for {formatDate(event.date)}</p>
                        <div className="flex flex-col gap-2">
                            ...
                            <button onClick={onClose}
                                className="w-full py-3 bg-surface-secondary text-primary rounded-xl font-bold text-xs hover:bg-surface-tertiary transition-colors">
                                Close
                            </button>
                        </div>
                    </div>
                    ...
                    <>
                        ...
                        <div className="p-6 border-b border-border-primary">
                            <div className="flex items-center justify-between mb-3">
                                <h2 className="text-lg font-black text-primary">Get Tickets</h2>
                            ...
                            <div className="flex items-center gap-3">
                                ...
                                <div>
                                    ...
                                    <p className="text-xs text-tertiary">{formatDate(event.date)} at {formatTime(event.date)}</p>
                        ...
                        {step === 'select' && (
                            <div className="p-6 space-y-4">
                                <p className="text-xs font-bold text-tertiary uppercase tracking-widest">Select Tickets</p>
                                {ticketTypes.map(ticket => (
                                    <div key={ticket.id} className="bg-surface-secondary rounded-2xl p-4 border border-border-primary">
                                        <div className="flex items-start justify-between mb-3">
                                            <div>
                                                <h4 className="text-sm font-bold text-primary">{ticket.name}</h4>
                                                {ticket.description && <p className="text-[11px] text-tertiary mt-0.5">{ticket.description}</p>}
                                            </div>
                                        ...
                                        <div className="flex items-center justify-between">
                                            ...
                                            <div className="flex items-center gap-3">
                                                ...
                                                <span className="w-6 text-center font-bold text-sm text-primary">{quantities[ticket.id] || 0}</span>
                                                <button
                                                    onClick={() => handleQuantity(ticket.id, 1)}
                                ...
                                <div className="pt-2 space-y-3">
                                    ...
                                    <div className="flex items-center justify-between text-sm">
                                        <span className="text-tertiary">Service fee</span>
                                        <span className="font-bold text-primary">${(totalPrice * 0.05).toFixed(2)}</span>
                                    </div>
                        ...
                        {step === 'confirm' && (
                            <div className="p-6 space-y-4">
                                <p className="text-xs font-bold text-tertiary uppercase tracking-widest">Confirm Purchase</p>
                                    ...
                                    {Object.entries(quantities).filter(([_, qty]) => qty > 0).map(([typeId, qty]) => {
                                        ...
                                        return (
                                            <div key={typeId} className="flex items-center justify-between text-sm">
                                                <span className="text-primary">{ticket.name} × {qty}</span>
                                                <span className="font-bold text-primary">${(ticket.price * qty).toFixed(2)}</span>
                                            </div>
                                        );
                                    })}
                                ...
                                <div className="flex items-center gap-3 p-3 bg-amber-500/5 rounded-xl border border-amber-500/10">
                                    ...
                                    <span className="text-xs text-tertiary">Secure payment via Stripe. Your tickets will be emailed and available in your profile.</span>
                                </div>

                                <div className="flex gap-3">
                                    <button onClick={() => setStep('select')}
                                        className="flex-1 py-3 bg-surface-secondary text-primary rounded-xl font-bold text-xs hover:bg-surface-tertiary transition-colors">
                                        Back
                                    </button>
                    ...
                    <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                        <div>
                            <div className="flex items-center gap-2 mb-2">
                                ...
                                <span className="text-xs text-tertiary font-semibold">{events.length} events</span>
                            </div>
                            ...
                            <p className="text-sm text-tertiary mt-1">Discover what's happening on the island. Get tickets, RSVP, and connect.</p>
                        </div>
                        <div className="flex items-center gap-3">
                    ...
                    <div className="flex gap-2 overflow-x-auto pb-1 mt-6 scrollbar-hide">
                        {(['upcoming', 'ongoing', 'past', 'all'] as FilterType[]).map(f => (
                            <button key={f} onClick={() => setFilter(f)}
                                className={`px-5 py-2.5 rounded-xl text-xs font-black uppercase tracking-widest whitespace-nowrap transition-all ${filter === f ? 'bg-accent-500 text-white shadow-lg shadow-accent-500/15' : 'bg-surface-elevated text-tertiary border border-border-primary hover:bg-surface-secondary'}`}>
            ...
            <section className="max-w-7xl mx-auto px-4 sm:px-6 py-8">
                {isLoading ? (
                                    ...
                                    <div className="flex-1 p-5 md:p-6">
                                        <div className="flex flex-col md:flex-row md:items-start md:justify-between gap-4">
                                            <div className="flex-1 min-w-0">
                                                ...
                                                <h3 className="text-lg font-black text-primary mb-1.5 hover:text-accent-400 transition-colors">
                                                    {event.title}
                                                </h3>
                                                <p className="text-sm text-tertiary line-clamp-2 mb-3">{event.description}</p>

                                                {/* Meta info */}
                                                <div className="flex flex-wrap gap-x-4 gap-y-1.5 text-xs font-medium text-tertiary">
                                                ...
                                                {event.user_rsvp === 'attending' ? (
                                                    ...
                                                    <div className="flex gap-2">
                                                        ...
                                                        <button onClick={() => handleRSVP(event.event_id, 'interested')}
                                                            className="px-4 py-2.5 bg-surface-secondary text-tertiary rounded-xl text-[10px] font-bold hover:bg-surface-tertiary transition-colors">
                                                            ★
                                                        </button>
                                                    </div>
                                                )}
                    ...
                    <div className="bg-surface-elevated rounded-2xl border border-border-primary p-16 text-center">
                        ...
                        <p className="text-sm text-tertiary mb-6">Try a different filter or create a new event.</p>
