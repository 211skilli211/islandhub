    ...
    const listingColumns: Column<any>[] = [
        ...
        { header: 'Status', accessor: (item) => (
            <span className={`px-2 py-1 rounded-full text-xs font-bold ${
                ...
                item.status === 'inactive' ? 'bg-surface-secondary text-ink-secondary' :
        ...
        <div className="space-y-6">
            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                <div>
                    ...
                    <p className="text-ink-tertiary dark:text-ink-tertiary">Manage all marketplace listings</p>
                </div>
                <button 
                    onClick={() => router.push('/create')} 
                    className="px-5 py-2.5 bg-accent-500 text-white rounded-xl font-bold hover:bg-accent-600 transition-colors"
                >
                    + Add Listing
                </button>
            </div>
