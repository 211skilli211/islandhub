...
const secondaryNavItems = [
  ...
  { id: 'tile-manager', label: 'Tile Manager', icon: Image, href: '/admin/assets/tile-manager', parent: 'media' },
          ...
          <div key={item.id}>
            <div className="flex items-center gap-1">
              <Link
                ...
                } ${active ? 'bg-theme-tertiary text-theme-primary font-bold' : 'text-theme-secondary hover:bg-theme-tertiary hover:text-theme-primary'}`}
              >
              ...
              {hasChildren && !isRail && !isMobile && (
                <button onClick={() => toggleGroup(item.id)} className="p-1 rounded-md hover:bg-theme-tertiary transition-colors">
                  <ChevronDown size={14} className={`text-theme-tertiary transition-transform ${isGroupExpanded ? 'rotate-180' : ''}`} />
                </button>
              )}
            </div>
                  ...
                  {hasChildren.items.map(sub => {
                    ...
                    return (
                      <Link
                        ...
                        } ${subActive ? 'text-teal-300 bg-teal-500/20' : 'text-theme-secondary hover:text-theme-primary hover:bg-theme-tertiary'}`}
                      >
  ...
  return (
    <div className="min-h-screen bg-surface-primary">
      <div className="lg:hidden sticky top-0 z-30 bg-surface-primary/80 backdrop-blur-lg border-b border-border-primary px-4 py-3 flex items-center justify-between">
        ...
        <span className="font-bold text-sm text-theme-primary">Admin</span>
        <div className="w-8" />
      </div>
      ...
      <div className="flex">
        ...
        <aside className={`fixed inset-y-0 left-0 z-50 w-[280px] bg-surface-elevated text-theme-primary flex flex-col border-r border-border-primary transition-transform duration-300 ease-in-out lg:hidden ${mobileOpen ? 'translate-x-0' : '-translate-x-full'}`}>
          ...
          <div className="flex items-center justify-between px-4 py-4 border-b border-border-primary shrink-0">
            <div className="flex items-center gap-2.5">
              <div className="w-7 h-7 rounded-lg bg-theme-tertiary flex items-center justify-center">
                <Settings size={15} className="text-theme-primary" />
              </div>
              <span className="font-bold text-sm text-theme-primary">Admin Panel</span>
            </div>
          ...
          {user && (
            <Link href="/profile" onClick={() => setMobileOpen(false)}
              ...
              <div className="w-7 h-7 rounded-full bg-theme-tertiary flex items-center justify-center shrink-0 overflow-hidden">
                {user.avatar_url ? <img src={user.avatar_url} alt="" className="w-full h-full object-cover" /> : <User size={14} className="text-theme-tertiary" />}
              </div>
              <div className="min-w-0">
                <div className="text-[12px] font-semibold text-theme-primary truncate">{user.name}</div>
                <div className="text-[10px] text-theme-tertiary">{user.role || 'Admin'}</div>
              </div>
            </Link>
          )}
          {renderNavItems(true)}
          ...
          <div className="p-3 border-t border-border-primary shrink-0 space-y-1">
            <Link href="/" onClick={() => setMobileOpen(false)}
              className="flex items-center gap-2.5 px-3 py-2 rounded-lg text-theme-secondary hover:bg-theme-tertiary hover:text-theme-primary transition-colors">
              <Home size={15} className="shrink-0" />
              <span className="text-[12px] font-medium">Back to Home</span>
            </Link>
            <button onClick={() => { handleLogout(); setMobileOpen(false); }}
              className="flex items-center gap-2.5 px-3 py-2 rounded-lg text-theme-tertiary hover:bg-red-500/20 hover:text-red-300 transition-colors w-full">
        ...
        <aside
          className="hidden lg:flex fixed left-0 top-0 bottom-0 z-[60] bg-surface-elevated text-theme-primary flex-col border-r border-border-primary transition-all duration-300"
        ...
        >
          ...
          <div className={`shrink-0 flex items-center border-b border-border-primary ${isRail ? 'justify-center px-0 py-4' : 'px-4 py-4'}`}>
            {!isRail && (
              <div className="flex items-center gap-2.5 min-w-0">
                ...
                <span className="font-bold text-sm tracking-tight truncate text-theme-primary">Admin</span>
              </div>
            )}
            {isRail && (
            ...
            <Link href="/profile"
              ...
              {!isRail && (
                <div className="min-w-0 flex-1">
                  <p className="font-semibold text-[12px] truncate text-theme-primary">{user.name}</p>
                  <p className="text-[10px] text-theme-tertiary capitalize">{user.role || 'Admin'}</p>
                </div>
              )}
            </Link>
          )}
          ...
          <div className={`shrink-0 border-b border-border-primary ${isRail ? 'py-2' : 'py-2 px-3'}`}>
            <Link href="/" className={`flex items-center text-theme-tertiary hover:text-theme-secondary transition-colors ${isRail ? 'justify-center' : 'gap-1.5'}`} title={isRail ? 'Back to Home' : undefined}>
              <ArrowLeft size={12} />
          ...
          <div className={`shrink-0 border-t border-border-primary ${isRail ? 'p-2' : 'p-3'}`}>
            <button onClick={handleLogout}
              className={`flex items-center rounded-lg text-theme-tertiary hover:bg-red-500/20 hover:text-red-300 transition-colors ${isRail ? 'justify-center p-2 w-full' : 'gap-2.5 px-3 py-2 w-full'}`}
