...
export default function IBTServiceDetailPage() {
        ...
        return (
            <main className="min-h-screen bg-surface-primary flex items-center justify-center">
                <div className="text-center">
                    <EmojiIcon emoji="üòï" size={48} className="text-6xl mb-4" />
    ...
    const icons: Record<string, string> = {
        ai_employees: 'ü§ñ', web_design: 'üíª', automation: '‚öôÔ∏è',
        api_integration: 'üîå', coop_membership: 'ü§ù',
    };
            ...
            <section className="bg-surface-tertiary py-16 px-6">
                <div className="max-w-4xl mx-auto">
                    ...
                    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
                        <div className="text-5xl mb-4">{icons[service.service_type] || 'üì¶'}</div>
            ...
            <div className="max-w-4xl mx-auto px-6 py-12">
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    {service.metadata.features.map((feature: string, i: number) => (
                                        <div key={i} className="flex items-center gap-3 p-3 bg-surface-primary rounded-xl">
                                            <EmojiIcon emoji="‚úì" size={16} className="w-6 h-6 rounded-full bg-accent-500/15 text-accent-400 flex items-center justify-center text-xs font-bold" />
                    ...
                    <div>
                        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }}
                            ...
                            {inquirySent ? (
                                <div className="text-center py-8">
                                    <EmojiIcon emoji="‚úÖ" size={40} className="text-4xl mb-4" />
                      ...
                      †          "      clAssN`me="w%full px-4 py-3 rg-surface-primary border border-boÚdep-primazq rounded/xl text-sm focuS:outline-none focws:border-teal-500"
  `   %       †           "    a    />
  ` !             b    †(    †  "   <senEcv
$                    `        †         valee={inquiryFoRm.budget_rangd} onChange={e }> seu	nquiryFgrm(p0=> ({"...p, budget_ra>ge: e.target.6alue }))}
  `     (             $                 c|assName="w-full px-4 py-3 bg-sur$ace-primery bovdEr bordmV-border-psimary rounde`-xh text-sm focuw:ouuline-Æone$focus:border-teil-400"
         $   `             $(   (   >
 `  "      (        ( "      0       $ $<option v!nuG="">Budwet Range</optkon>
 `  4 `              `  "   "0  `       <option value?"< $1,000">ulder $1=000</option>
           $          !            (`0  <oqtion value="$1,002 - d5l000"æ$5,000 - $5,02</option~
