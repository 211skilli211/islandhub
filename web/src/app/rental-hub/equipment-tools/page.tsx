        ...
        <div className="min-h-screen bg-surface-elevated">
            ...
            <section className="bg-surface-primary py-16">
                <div className="max-w-7xl mx-auto px-6">
                    <div className="flex justify-between items-end mb-12">
                        <div>
                            <h2 className="text-2xl font-black text-ink-primary tracking-tight italic uppercase">Trusted Equipment Vendors</h2>
                            <p className="text-ink-tertiary font-medium">Reliable partners for tools and utility rentals</p>
                        </div>
                    </div>
                    <div className="flex gap-6 overflow-x-auto pb-6 scrollbar-hide">
                        {vendors.length > 0 ? vendors.map(v => (
                            <Link key={v.id} href={`/store/${v.slug}`} className="shrink-0 w-64 p-8 bg-surface-elevated rounded-[2.5rem] border border-border-primary hover:shadow-2xl transition-all group">
                                ...
                                <p className="text-ink-secondary text-[10px] font-black uppercase tracking-widest">{v.subtype || 'Utility Expert'}</p>
                            </Link>
                        )) : (
                            <div className="text-ink-tertiary font-bold italic py-4">Showcasing top utility stores soon...</div>
                        )}
                    </div>
                </div>
            </section>
            ...
            <div className="max-w-7xl mx-auto px-6 py-24">
                <div className="flex flex-col md:flex-row justify-between items-end mb-16 gap-8">
                    <div>
                        <h2 className="text-3xl font-black text-ink-primary tracking-tight italic uppercase">Tool Catalogue</h2>
                        <p className="text-ink-tertiary font-medium">Filter by utility category</p>
                    </div>
                    <div className="flex gap-2 p-1 bg-surface-primary rounded-2xl border border-border-primary">
                        {['All', 'Tools', 'Event', 'Marine', 'Power'].map(f => (
                            <button
                                ...
                                className={`px-6 py-3 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${activeFilter === f ? 'bg-surface-elevated text-ink-primary shadow-sm' : 'text-ink-tertiary hover:text-ink-primary'}`}
                            >
                                {f}
                            </button>
                        ))}
                ...
                {filteredListings.length === 0 && (
                    <div className="text-center py-32 bg-surface-primary rounded-[4rem] border-2 border-dashed border-border-primary">
                        <span className="text-5xl mb-6 block">🛠️</span>
                        <h3 className="text-2xl font-black text-ink-primary mb-2">Catalogue Empty</h3>
                        <p className="text-ink-tertiary font-bold italic">Adjust your filters to see more utility options.</p>
                    </div>
                )}
            </div>

            <div className="max-w-7xl mx-auto px-6 pb-24 text-center">
                <Link href="/rentals" className="inline-flex items-center gap-2 text-ink-tertiary font-bold hover:text-ink-primary transition-colors">
