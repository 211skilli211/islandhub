      ...
      <section className="max-w-6xl mx-auto px-4 py-20 -mt-10 relative z-20">
        <div className="grid md:grid-cols-2 gap-8">
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              <div className="grid md:grid-cols-2 gap-4">
                <div>
                  ...
                  <input type="text" required className="w-full px-4 py-4 rounded-2xl border border-slate-200 focus:border-teal-500 focus:ring-0 bg-slate-50" placeholder="Your name" value={formData.name} onChange={e => setFormData({...formData, name: e.target.value})} />
                </div>
                <div>
                  <label className="block text-xs font-black text-slate-500 uppercase tracking-widest mb-2">Email</label>
                  <input type="email" required className="w-full px-4 py-4 rounded-2xl border border-slate-200 focus:border-teal-500 focus:ring-0 bg-slate-50" placeholder="you@example.com" value={formData.email} onChange={e => setFormData({...formData, email: e.target.value})} />
                </div>
              </div>
              <div>
                <label className="block text-xs font-black text-slate-500 uppercase tracking-widest mb-2">Subject</label>
                <select className="w-full px-4 py-4 rounded-2xl border border-slate-200 focus:border-teal-500 focus:ring-0 bg-slate-50" value={formData.subject} onChange={e => setFormData({...formData, subject: e.target.value})}>
              ...
              <div>
                ...
                <textarea rows={5} required className="w-full px-4 py-4 rounded-2xl border border-slate-200 focus:border-teal-500 focus:ring-0 bg-slate-50" placeholder="How can we help?" value={formData.message} onChange={e => setFormData({...formData, message: e.target.value})} />
              </div>
          ...
          <div className="space-y-8">
              ...
              <div className="space-y-6">
                <div className="flex items-start gap-4">
                  ...
                  <div>
                    ...
                    <a href="mailto:businesstrends869@gmail.com" className="text-teal-600 hover:underline">businesstrends869@gmail.com</a>
                  </div>
                </div>
                <div className="flex items-start gap-4">
                  <div className="w-12 h-12 bg-teal-50 rounded-2xl flex items-center justify-center text-2xl">📱</div>
                  <div>
                    <p className="font-black text-slate-900 text-sm uppercase tracking-widest">Phone</p>
                    <a href="tel:+18697639919" className="text-teal-600 hover:underline">+1 (869) 763-9919</a>
                  </div>
                </div>
                <div className="flex items-start gap-4">
                  <div className="w-12 h-12 bg-teal-50 rounded-2xl flex items-center justify-center text-2xl">💬</div>
                  <div>
                    ...
                    <a href="https://wa.me/18697639919" target="_blank" rel="noopener noreferrer" className="text-teal-600 hover:underline">Chat with us</a>
                  </div>
                </div>
                <div className="flex items-start gap-4">
                  <div className="w-12 h-12 bg-teal-50 rounded-2xl flex items-center justify-center text-2xl">🏝️</div>
                  <div>
                    <p className="font-black text-slate-900 text-sm uppercase tracking-widest">IBT Co-ops</p>
                    <a href="https://chat.whatsapp.com/IfkJFCpgKRn9dOLaAUzOxW" target="_blank" rel="noopener noreferrer" className="text-teal-600 hover:underline">Join WhatsApp Community</a>
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className="bg-white p-10 rounded-[3rem] shadow-lg border border-slate-100 text-center">
              <h3 className="text-xl font-black text-slate-900 mb-4">Meet the Founder</h3>
              <p className="text-slate-600 mb-4">Learn more about the vision behind IslandHub.</p>
              <Link href="/about" className="inline-flex items-center gap-2 text-teal-600 font-bold text-sm hover:text-teal-700 transition-colors">
                About N. J. Robin →
              </Link>
            </div>
          </div>
