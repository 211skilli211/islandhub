...
export default function HeroAssetTab() {
    ...
    type AssetType = 'image' | 'video' | 'shader' | 'particle';
    ...
    const handleSave = async () => {
        if ((assetType === 'image' || assetType === 'video') && !assetUrl) {
            toast.error('Please provide an asset URL or upload a file');
            return;
        }
        if ((assetType === 'shader' || assetType === 'particle') && !assetUrl) {
            // shader/particle don't need an assetUrl
        }
                                ...
                                >
                                    ...
                                    <div>
                                        ...
                                        <div className="grid grid-cols-2 gap-2 p-1 bg-slate-50 rounded-2xl border border-slate-100">
                                            {(['image', 'video', 'shader', 'particle'] as AssetType[]).map(t => (
                                                <button
                                                    key={t}
                                        ...
                                        {(assetType === 'shader' || assetType === 'particle') && (
                                            <p className="text-[9px] text-teal-600 font-medium mt-2 ml-1">
                                                {assetType === 'shader'
                                                  ? 'Animated ocean shader — no upload needed, configure colors below'
                                                  : 'Animated particle field — no upload needed, configure theme below'}
                                            </p>
                                        )}
                                    </div>
                                            ...
                                            <div>
                                                ...
                                                <div className={`relative border-2 border-dashed rounded-2xl p-6 text-center transition-all ${uploading ? 'bg-slate-50 border-slate-200' : 'hover:border-teal-500 bg-slate-50 border-slate-100'}`}>
                                                    ...
                                                    {uploading ? (
                                                        ...
                                                        <div className="flex flex-col items-center">
                                                            <span className="text-3xl mb-2">📁</span>
                                                            <span className="text-[10px] font-black uppercase tracking-widest text-slate-600">Click or Drag {assetType === 'video' ? 'Video' : 'Image'}</span>
                                                            <span className="text-[10px] text-slate-400 font-medium mt-1">Supports MP4, JPG, PNG (Max 50MB)</span>
                                                        </div>
                                                    )}
                                                </div>
                                    ...
                                    {assetType === 'shader' && (
                                        <div className="space-y-4 p-4 bg-teal-50/50 rounded-2xl border border-teal-100">
                                            <label className="block text-[10px] font-black uppercase tracking-widest text-teal-600">Shader Configuration</label>
                                            <div>
                                                <label className="block text-[9px] font-bold text-slate-500 mb-1 ml-1">Palette Colors (up to 4, hex)</label>
                                                <div className="flex gap-2">
                                                    {[0,1,2,3].map(i => (
                                                        <div key={i} className="flex-1">
                                                            <input
                                                                type="color"
                                                                value={(styleConfig?.shaderColors?.[i]) || ['#0A1628','#0F2744','#14B8A6','#F59E0B'][i]}
                                                                onChange={(e) => {
                                                                    const colors = [...(styleConfig?.shaderColors?.[i] ? styleConfig.shaderColors : ['#0A1628','#0F2744','#14B8A6','#F59E0B'])];
                                                                    colors[i] = e.target.value;
                                                                    setStyleConfig({ ...styleConfig, shaderColors: colors });
                                                                }}
                                            ...
                                            <div className="grid grid-cols-2 gap-4">
                                                <div>
                                                    <label className="block text-[9px] font-bold text-slate-500 mb-1 ml-1">Speed: {styleConfig?.shaderSpeed ?? 1}</label>
                                                    <input type="range" min="0.1" max="3" step="0.1"
                                                        value={styleConfig?.shaderSpeed ?? 1}
                                                        onChange={(e) => setStyleConfig({ ...styleConfig, shaderSpeed: parseFloat(e.target.value) })}
                                                        className="w-full h-2 bg-teal-100 rounded-lg appearance-none cursor-pointer accent-teal-500"
                                                    />
                                                </div>
                                                <div>
                                                    <label className="block text-[9px] font-bold text-slate-500 mb-1 ml-1">Intensity: {styleConfig?.shaderIntensity ?? 1}</label>
                                                    <input type="range" min="0.2" max="2" step="0.1"
                                                        value={styleConfig?.shaderIntensity ?? 1}
                                                        onChange={(e) => setStyleConfig({ ...styleConfig, shaderIntensity: parseFloat(e.target.value) })}
                                                        className="w-full h-2 bg-teal-100 rounded-lg appearance-none cursor-pointer accent-teal-500"
                                                    />
                                                </div>
                                            </div>
                                    ...
                                    {assetType === 'particle' && (
                                        <div className="space-y-4 p-4 bg-amber-50/50 rounded-2xl border border-amber-100">
                                            <label className="block text-[10px] font-black uppercase tracking-widest text-amber-600">Particle Configuration</label>
                                            <div>
                                                <label className="block text-[9px] font-bold text-slate-500 mb-1 ml-1">Theme</label>
                                                <div className="grid grid-cols-2 gap-2">
                                                    {(['tropical', 'ocean', 'aurora', 'fireflies'] as const).map(t => (
                                                        <button key={t}
                                                            onClick={() => setStyleConfig({ ...styleConfig, particleTheme: t })}
                                                            className={`py-2 rounded-xl text-[9px] font-black uppercase tracking-widest transition-all ${
                                                                (styleConfig?.particleTheme || 'tropical') === t
                                            ...
                                            <div className="grid grid-cols-2 gap-4">
                                                <div>
                                                    <label className="block text-[9px] font-bold text-slate-500 mb-1 ml-1">Count: {styleConfig?.particleCount ?? 80}</label>
                                                    <input type="range" min="20" max="200" step="10"
                                                        value={styleConfig?.particleCount ?? 80}
                                                        onChange={(e) => setStyleConfig({ ...styleConfig, particleCount: parseInt(e.target.value) })}
                                                        className="w-full h-2 bg-amber-100 rounded-lg appearance-none cursor-pointer accent-amber-500"
                                                    />
                                                </div>
                                                <div>
                                                    <label className="block text-[9px] font-bold text-slate-500 mb-1 ml-1">Speed: {styleConfig?.particleSpeed ?? 1}</label>
                                                    <input type="range" min="0.2" max="3" step="0.1"
                                                        value={styleConfig?.particleSpeed ?? 1}
                                                        onChange={(e) => setStyleConfig({ ...styleConfig, particleSpeed: parseFloat(e.target.value) })}
                                                        className="w-full h-2 bg-amber-100 rounded-lg appearance-none cursor-pointer accent-amber-500"
                                                    />
                                                </div>
                                            </div>
                ...
                <div className="sticky top-8">
                    ...
                    <div className="relative aspect-video rounded-[3rem] overflow-hidden border-8 border-white shadow-2xl shadow-slate-300" style={{ backgroundColor: styleConfig.bgColor || '#000000' }}>
                        ...
                        >
                            {/* Shader Preview */}
                            {assetType === 'shader' && (
                                <div className="absolute inset-0 overflow-hidden" style={{ background: 'linear-gradient(135deg, #0A1628 0%, #0F2744 40%, #14B8A6 80%, #F59E0B 100%)' }}>
                                    <div className="absolute inset-0 opacity-60" style={{ background: 'radial-gradient(ellipse at 30% 50%, rgba(20,184,166,0.3) 0%, transparent 70%)' }} />
                                    <div className="absolute inset-0 opacity-40" style={{ background: 'radial-gradient(ellipse at 70% 30%, rgba(245,158,11,0.2) 0%, transparent 60%)' }} />
                                    <div className="absolute bottom-0 left-0 right-0 h-1/3 bg-gradient-to-t from-[#0A1628] to-transparent" />
                                    <div className="absolute top-3 left-3 text-[8px] font-bold text-teal-400 bg-black/40 px-2 py-1 rounded-lg">
                                        🌊 Shader Preview
                                    </div>
                                </div>
                            )}
                            ...
                            {assetType === 'particle' && (
                                <div className="absolute inset-0 overflow-hidden bg-[#0A1628]">
                                    <div className="absolute inset-0" style={{
                                        background: 'radial-gradient(circle at 20% 30%, rgba(245,158,11,0.15) 0%, transparent 40%), radial-gradient(circle at 80% 60%, rgba(20,184,166,0.15) 0%, transparent 40%), radial-gradient(circle at 50% 80%, rgba(6,182,212,0.1) 0%, transparent 40%)'
                                    }} />
                                    {[...Array(12)].map((_, i) => (
                                        <div key={i} className="absolute rounded-full" style={{
                                            ...
                                            top: `${Math.random() * 100}%`,
                                            backgroundColor: ['#F59E0B', '#14B8A6', '#06B6D4', '#A855F7'][i % 4],
                                            opacity: Math.random() * 0.5 + 0.2,
                                        }} />
                                    ))}
                                    <div className="absolute top-3 left-3 text-[8px] font-bold text-amber-400 bg-black/40 px-2 py-1 rounded-lg">
                                        ✨ Particle Preview
                                    </div>
                                </div>
                            )}
