            ...
            {formData.image_url ? (
                <div className="relative h-48">
                    ...
                    <div 
                        ...
                        style={{ 
                            background: `linear-gradient(to top, ${formData.overlay_color || '#000000'} ${(formData.overlay_opacity || 0) + 60}%, transparent)` 
                        }} 
                    />
                            ...
                            <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
                                {LAYOUT_PRESETS.map(preset => (
                                    <button
                                        key={preset.id}
                                        type="button"
                                        onClick={() => setFormData({ ...formData, layout_preset: preset.id as 'split' | 'neon' | 'overlay' | 'standard' | 'glow' | 'hero' | 'minimal' | 'floating' | 'glass' })}
                                ...
                                <div className="flex bg-surface-secondary p-1 rounded-xl">
                                    {['left', 'center', 'right'].map((align) => (
                                        <button
                                            key={align}
                                            type="button"
                                            onClick={() => setFormData({ ...formData, alignment: align as 'left' | 'center' | 'right' })}
                                    ...
                                    <div className="grid grid-cols-3 gap-2">
                                        {TEXTURE_PATTERNS.map(pattern => (
                                            <button
                                                key={pattern.id}
                                                type="button"
                                                onClick={() => setFormData({ ...formData, texture_pattern: pattern.id as 'none' | 'dots' | 'lines' | 'waves' | 'noise' | 'gradient' })}
