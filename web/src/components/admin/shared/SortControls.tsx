...
const SortControls: React.FC<SortControlsProps> = ({ sort, onSortChange, options }) => {
    return (
        <div className="flex items-center gap-2 bg-surface-elevated p-1 rounded-lg border border-border-primary">
            <span className="text-xs font-bold text-ink-tertiary uppercase ml-2 mr-1">Sort By</span>
            <select
                value={sort.sortBy}
                onChange={(e) => onSortChange({ ...sort, sortBy: e.target.value })}
                className="text-sm font-bold text-ink-secondary bg-transparent border-none focus:ring-0 cursor-pointer"
            >
                {options.map((opt) => (
            ...
            <div className="h-4 w-px bg-surface-tertiary mx-1" />
