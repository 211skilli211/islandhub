...
function StatusBadge({ status }: { status: string }) {
    const colors: Record<string, string> = {
        online: 'bg-accent-500/100 shadow-accent-500/20',
        offline: 'bg-[#e11d48]/50 shadow-rose-500/20',
        unknown: 'bg-sand-500/50 shadow-amber-500/20',
    };
    return (
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 p-8 bg-ink-primary rounded-[2.5rem] shadow-2xl border border-border-primary relative overflow-hidden group">
                ...
                <div className="relative z-10">
                    ...
                    <div className="flex flex-wrap items-center gap-4">
                        <div className="flex items-center gap-2 px-3 py-1.5 bg-surface-elevated/5 rounded-full border border-white/5 backdrop-blur-md">
                            <span className="text-[10px] font-black uppercase tracking-widest text-ink-tertiary">Provider</span>
                            <StatusBadge status={providerReady ? 'online' : 'offline'} />
                        </div>
                        <div className="flex items-center gap-2 px-3 py-1.5 bg-surface-elevated/5 rounded-full border border-white/5 backdrop-blur-md">
                            <span className="text-[10px] font-black uppercase tracking-widest text-ink-tertiary">Gateway</span>
                        ...
                        <button 
                            ...
                            className={`px-3 py-1.5 rounded-full border text-[10px] font-black uppercase tracking-widest transition-all ${
                                gatewayEnabled 
                                    ? 'bg-accent-500/100/20 border-teal-500 text-accent-400' 
                                    : 'bg-[#e11d48]/50/20 border-[#e11d48] text-[#fb7185]'
                            }`}
                        >
                ...
                <div className="relative z-10 flex gap-3">
                    <button
                        ...
                        className="px-6 py-3 bg-surface-elevated/5 hover:bg-surface-elevated/10 text-white rounded-2xl text-[10px] font-black uppercase tracking-[0.2em] transition-all border border-white/10 active:scale-95"
                    >
                        Sync Telemetry
                    </button>
                    <button
                        onClick={() => setActiveSection('chat')}
                        className="px-6 py-3 bg-accent-500 hover:bg-accent-500/100 text-white rounded-2xl text-[10px] font-black uppercase tracking-[0.2em] shadow-lg shadow-teal-600/20 transition-all active:scale-95"
                    >
                        Access Console
                    </button>
                </div>
                            ...
                            <div className="space-y-8">
                                {!providerReady && (
                                    <div className="p-6 bg-[#e11d48]/5 dark:bg-[#e11d48]/50/10 border border-[#e11d48]/20 dark:border-[#e11d48]/20 rounded-3xl text-xs font-bold text-[#e11d48] flex items-center gap-4">
                                        ...
                                        <button onClick={() => setActiveSection('providers')} className="ml-auto px-4 py-2 bg-[#e11d48] text-white rounded-xl uppercase tracking-tighter shadow-lg shadow-rose-600/20">Configure Bridge</button>
                                    </div>
                                )}
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                                    {agents.map(agent => (
                                        <div key={agent.name} className="group bg-surface-elevated dark:bg-ink-primary rounded-[2.5rem] p-1 border border-border-primary dark:border-border-primary shadow-sm hover:shadow-2xl transition-all relative overflow-hidden">
                                            <div className="p-7">
                                                <div className="flex items-center justify-between mb-8">
                                                    <div className="w-14 h-14 bg-surface-secondary dark:bg-surface-tertiary rounded-3xl flex items-center justify-center text-3xl shadow-inner group-hover:rotate-6 transition-transform">
                                                        {agent.icon}
                                                    </div>
                                                    <StatusBadge status={agent.status} />
                                                </div>

                                                <h4 className="text-lg font-black text-ink-primary dark:text-white mb-1">{agent.displayName}</h4>
                                                <p className="text-[10px] font-mono text-ink-tertiary uppercase tracking-widest mb-6">{agent.model}</p>

                                                <div className="grid grid-cols-2 gap-4 mb-8">
                                                    <div className="p-4 bg-surface-secondary dark:bg-surface-tertiary/50 rounded-2xl">
                                                        <p className="text-[8px] font-black uppercase tracking-widest text-ink-tertiary mb-1">Messages</p>
                                                        <p className="text-sm font-black text-accent-400">{agent.interactions24h}</p>
                                                    </div>
                                                    <div className="p-4 bg-surface-secondary dark:bg-surface-tertiary/50 rounded-2xl">
                                                ...
                                                <button
                                                    ...
                                                    className="w-full py-4 bg-ink-primary dark:bg-surface-tertiary hover:bg-accent-500 text-white rounded-2xl text-[10px] font-black uppercase tracking-[0.2em] transition-all active:scale-95"
                                                >
                                                    Open Uplink
                                                </button>
                                            </div>
                                ...
                                <div className="flex-1 flex flex-col bg-surface-secondary dark:bg-ink-950/20">
                                    ...
                                    <div className="px-8 py-5 border-b border-border-primary dark:border-border-primary bg-surface-elevated/50 dark:bg-ink-primary/50 backdrop-blur-md flex items-center justify-between">
                                        <div className="flex items-center gap-3">
                                            ...
                                            <span className="text-[11px] font-black text-ink-primary dark:text-ink-tertiary uppercase tracking-widest">Encypted Session: {chatAgent}</span>
                                        </div>
                                        <button onClick={() => { setConversationId(''); setChatMessages([{ role: 'agent', content: 'Session history purged. Neural bridge reset.' }]); }} className="text-[9px] font-black text-[#e11d48] uppercase tracking-widest">Purge History</button>
                                    </div>
                                    ...
                                    <div ref={chatScrollRef} className="flex-1 p-8 space-y-6 overflow-y-auto custom-scrollbar">
                                        {chatMessages.map((m, i) => (
                                            ...
                                            >
                                                <div className={`max-w-[85%] px-6 py-4 rounded-3xl shadow-sm text-sm font-medium leading-relaxed ${m.role === 'user'
                                                    ? 'bg-ink-primary text-white rounded-tr-none shadow-xl shadow-slate-900/20'
                                                    : 'bg-surface-elevated dark:bg-surface-tertiary text-ink-secondary dark:text-ink-tertiary border border-border-primary dark:border-border-primary rounded-tl-none'
                                                    }`}>
                                                    {m.content}
                                                </div>
                                            </motion.div>
                                        ...
                                        {chatTyping && (
                                            <div className="flex justify-start">
                                                <div className="bg-surface-elevated dark:bg-surface-tertiary px-6 py-4 rounded-3xl rounded-tl-none border border-border-primary dark:border-border-primary flex gap-2">
                                                    <span className="w-2 h-2 bg-accent-500/100 rounded-full animate-bounce" />
                                                    <span className="w-2 h-2 bg-accent-500/100 rounded-full animate-bounce" style={{ animationDelay: '0.2s' }} />
                                                    <span className="w-2 h-2 bg-accent-500/100 rounded-full animate-bounce" style={{ animationDelay: '0.4s' }} />
                                                </div>
                                            </div>
                                        )}
                                    </div>

                                    <div className="p-6 bg-surface-elevated dark:bg-ink-primary border-t border-border-primary dark:border-border-primary">
                            ...
                            <div className="space-y-8">
                                <div className="flex justify-between items-center bg-ink-primary rounded-3xl p-6 border border-border-primary shadow-xl">
                                    ...
                                    <button
                                        ...
                                        className="px-8 py-3 bg-accent-500 hover:bg-accent-500/100 text-white rounded-2xl text-[10px] font-black uppercase tracking-widest transition-all"
                                    >
                                ...
                                <div className="grid grid-cols-1 gap-6">
                                    {agentConfigs.map(agent => (
                                        <div key={agent.agent_id} className="bg-surface-elevated dark:bg-ink-primary rounded-[2.5rem] p-8 border border-border-primary dark:border-border-primary shadow-sm hover:shadow-xl transition-all">
                                            <div className="flex items-center justify-between mb-10">
                                                <div className="flex items-center gap-6">
                                                    <div className="w-16 h-16 bg-surface-secondary dark:bg-surface-tertiary rounded-3xl flex items-center justify-center text-4xl shadow-inner">
                                                        {agent.icon}
                                                    </div>
                                                    <div>
                                            ...
                                            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 mt-6">
                                                <div>
                                                    <h5 className="text-[9px] font-black uppercase tracking-[0.2em] text-ink-tertiary mb-2">Capabilities Descriptor</h5>
                                                    <p className="text-sm text-ink-secondary dark:text-ink-tertiary leading-relaxed">{agent.description}</p>
                                                </div>
                                                <div className="bg-surface-secondary dark:bg-black/20 rounded-2xl p-4 overflow-hidden group">
                                                    <h5 className="text-[9px] font-black uppercase tracking-[0.2em] text-ink-tertiary mb-2 flex justify-between">
                            ...
                            <div className="bg-surface-elevated dark:bg-ink-primary rounded-[3rem] border border-border-primary dark:border-border-primary p-10 shadow-xl">
                                {activeSection === 'audit' && (
                                    <div className="space-y-8">
                                        <div className="flex justify-between items-center">
                                            <h3 className="font-black text-2xl text-ink-primary dark:text-white">Neural Ledger</h3>
                                            <button onClick={fetchAudit} className="text-[10px] text-accent-400 font-black uppercase tracking-widest bg-accent-500/10 dark:bg-accent-500/100/10 px-4 py-2 rounded-xl">Sync Ledger</button>
                                        </div>
                                        <div className="divide-y divide-slate-100 dark:divide-slate-800">
                                            {auditLogs.length === 0 ? (
                                                <div className="p-20 text-center text-ink-tertiary opacity-50">No ledger entries detected in this epoch.</div>
                                            ) : (
                                                auditLogs.map(log => (
                                                    <div key={log.id} className="py-5 flex items-center gap-6 group hover:bg-surface-secondary dark:hover:bg-surface-elevated/5 px-4 rounded-2xl transition-all">
                                                        <span className="text-[10px] font-mono text-ink-tertiary">#{log.id}</span>
                                                        <div className="flex flex-col">
                                                            <span className="text-xs font-black uppercase tracking-widest text-ink-primary dark:text-ink-tertiary">{log.action?.replace(/_/g, ' ')}</span>
                                                            <span className="text-[10px] font-bold text-ink-tertiary italic">By {log.admin_name || 'System Core'}</span>
                                                        </div>
                                                        <span className="text-[10px] text-ink-tertiary ml-auto font-mono">{new Date(log.created_at).toLocaleString()}</span>
                                                    </div>
                                                ))
                                            )}
                                        </div>
                                        ...
                                        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                                            {Object.entries(settings).map(([key, value]) => (
                                                <div key={key} className="space-y-3">
                                                    <label className="block text-[10px] font-black uppercase tracking-[0.2em] text-ink-tertiary">{key.replace(/_/g, ' ')}</label>
                                                    {key === 'autonomy_level' ? (
                                                        <select disabled={user?.role !== 'super-admin'} value={value} onChange={e => setSettings({ ...settings, [key]: e.target.value })}
                                                            className="w-full px-6 py-4 bg-surface-secondary dark:bg-surface-tertiary rounded-2xl text-sm border-2 border-transparent focus:border-teal-500 font-bold dark:text-white outline-none transition-all">
                                ...
                                {activeSection === 'providers' && (
                                    <div className="space-y-8">
                                        ...
                                        {showNewProvider && (
                                            <div className="bg-surface-secondary dark:bg-surface-elevated/5 p-8 rounded-4xl border border-[#14b8a6]/30 animate-in slide-in-from-top-4 duration-500">
                                                ...
                                                <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                                                    ...
                                                    <input placeholder="Display Name (e.g. OpenRouter Core)" className="bg-surface-elevated dark:bg-ink-primary p-4 rounded-2xl text-xs font-bold dark:text-white" value={newProviderData.display_name} onChange={e => setNewProviderData({ ...newProviderData, display_name: e.target.value })} />
                                        ...
                                        <div className="grid grid-cols-1 gap-6">
                                            {providers.map(p => (
                                                <div key={p.provider_name} className="p-8 bg-surface-secondary dark:bg-surface-elevated/5 rounded-[2.5rem] border border-border-primary dark:border-white/5 hover:ring-2 hover:ring-teal-500/20 transition-all">
                                                    <div className="flex items-center justify-between mb-8">
                                                        <div>
                                                            <h4 className="text-xl font-black dark:text-white mb-1">{p.display_name}</h4>
                                                            <span className="text-[10px] font-mono text-ink-tertiary tracking-widest uppercase italic">{p.api_base_url || 'Internal Mesh'}</span>
                                                        </div>
                                                        <div className="flex items-center gap-3">
                                                            <button 
                                                                ...
                                                                className={`px-4 py-2 rounded-xl text-[10px] font-black uppercase ${
                                                                    p.is_active 
                                                                        ? 'bg-[#e11d48]/10 text-[#e11d48] hover:bg-[#e11d48]/15' 
                                                    ...
                                                    <div className="grid grid-cols-3 gap-8 mb-8">
                                                        ...
                                                        <div className="bg-surface-elevated dark:bg-ink-primary p-6 rounded-3xl">
                                                            ...
                                                            <p className="text-lg font-black text-accent-400">${p.monthly_budget_usd}</p>
                                                        </div>
                                                        <div className="bg-surface-elevated dark:bg-ink-primary p-6 rounded-3xl">
                                                            <p className="text-[8px] font-black uppercase text-ink-tertiary mb-1">Neural Models</p>
                                                            <p className="text-lg font-black text-[#a5b4fc]0">{(p.models_available || []).length}</p>
                                                        </div>
                                                    </div>
                                                    ...
                                                    <div className="flex gap-3">
                                                        ...
                                                        {editingProvider === p.provider_name ? (
                                                            <div className="flex gap-2 flex-1 bg-surface-elevated dark:bg-ink-primary p-2 rounded-2xl ring-2 ring-teal-500/30">
                                                                <input 
                                                                    type="password" 
                                                                    value={newApiKey} 
                                                            ...
                                                            <button 
                                                                ...
                                                                className="px-6 py-3 bg-surface-tertiary text-white rounded-2xl text-[10px] font-black uppercase tracking-widest"
                                                            >
                                                ...
                                                workflows.map(wf => (
                                                    <div key={wf.workflow_id} className="bg-surface-elevated dark:bg-ink-primary rounded-4xl p-8 border border-border-primary dark:border-border-primary shadow-sm hover:shadow-xl transition-all">
                                                        ...
                                                        <div className="flex items-center gap-2">
                                                            ...
                                                            <span className="text-[9px] font-bold text-ink-tertiary uppercase ml-2">{JSON.parse(wf.steps || '[]').length} Stages</span>
                                                        </div>
                                                    </div>
                                                ))
                                            )}
                                ...
                                <div className="p-6 bg-surface-secondary dark:bg-surface-elevated/5 rounded-3xl space-y-6">
                                    ...
                                    <div>
                                        ...
                                        <input value={editingAgent.display_name} onChange={e => setEditingAgent({ ...editingAgent, display_name: e.target.value })}
                                            className="w-full px-4 py-3 bg-surface-elevated dark:bg-ink-primary rounded-xl text-sm font-bold dark:text-white border border-border-primary dark:border-white/5" />
                                    </div>
                                    <div>
                                        <label className="block text-[9px] font-black uppercase tracking-widest text-ink-tertiary mb-2">Unified Identifier</label>
                                        <input value={editingAgent.agent_id} disabled={!showNewAgent} onChange={e => setEditingAgent({ ...editingAgent, agent_id: e.target.value.toLowerCase().replace(/\s/g, '_') })}
                                            className="w-full px-4 py-3 bg-surface-elevated dark:bg-ink-primary rounded-xl text-xs font-mono dark:text-ink-tertiary border border-border-primary dark:border-white/5 disabled:opacity-50" />
                                    </div>
                                    <div>
                                        <label className="block text-[9px] font-black uppercase tracking-widest text-ink-tertiary mb-2">Language Model Base</label>
                                        <input value={editingAgent.model} onChange={e => setEditingAgent({ ...editingAgent, model: e.target.value })}
                                            className="w-full px-4 py-3 bg-surface-elevated dark:bg-ink-primary rounded-xl text-xs font-mono dark:text-ink-tertiary border border-border-primary dark:border-white/5" />
                                    </div>
                                </div>
                            </div>
                            ...
                            <div className="lg:col-span-2 space-y-8">
                                <div className="space-y-3">
                                    <div className="flex justify-between items-center px-2">
                                        <label className="block text-[9px] font-black uppercase tracking-widest text-ink-tertiary">Neural Directive (System Prompt)</label>
                                        <span className="text-[9px] font-mono text-[#a5b4fc]0">Character & Skill Protocol</span>
                                    </div>
                                    <textarea
                                    ...
                                    <div className="flex gap-4">
                                        ...
                                        <div className="flex-1">
                                            ...
                                            <input type="number" value={editingAgent.max_tokens} onChange={e => setEditingAgent({ ...editingAgent, max_tokens: parseInt(e.target.value) })}
                                                className="w-full px-4 py-2 bg-surface-secondary dark:bg-black/20 rounded-xl text-xs font-mono dark:text-white border border-border-primary dark:border-white/5" />
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div className="flex gap-4 pt-6 border-t border-border-primary dark:border-border-primary">
                                    ...
                                    <button onClick={() => { setEditingAgent(null); setShowNewAgent(false); }}
                                        className="px-10 py-5 bg-surface-secondary dark:bg-surface-tertiary text-ink-tertiary rounded-2xl font-black uppercase text-xs tracking-widest hover:bg-surface-secondary transition-all">
                                        Discard Changes
                                    </button>
                                </div>
                            </div>
