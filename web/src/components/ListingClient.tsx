                                                ...
                                                <span className="block mt-1 font-medium text-ink-primary">
                                                    {Array.isArray(value) ? (
                                                        <div className="flex flex-wrap gap-1 mt-1">
                                                            {value.map((v, i) => {
                                                                const displayValue = typeof v === 'object' && v !== null ? String((v as Record<string, unknown>).label || (v as Record<string, unknown>).name || JSON.stringify(v)) : String(v);
                                                                return (
                                                        ...
                                                        String((value as Record<string, unknown>).label || (value as Record<string, unknown>).name || JSON.stringify(value))
                                                    ) : String(value ?? '')}
                                                </span>
                                            </div>
                                        );
