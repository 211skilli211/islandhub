...
export default function VendorPromotions({ storeId }: VendorPromotionsProps) {
    ...
    const getStatusBadge = (status: Promotion['approval_status']) => {
        switch (status) {
            case 'approved':
                return <span className="flex items-center gap-1 px-2 py-1 bg-emerald-500/10 text-emerald-400 rounded-lg text-[10px] font-black uppercase tracking-widest"><CheckCircle size={12} /> Approved</span>;
            case 'pending':
                return <span className="flex items-center gap-1 px-2 py-1 bg-sand-500/5 text-sand-500 rounded-lg text-[10px] font-black uppercase tracking-widest"><Clock size={12} /> Pending</span>;
            case 'rejected':
                return <span className="flex items-center gap-1 px-2 py-1 bg-[#e11d48]/5 text-[#e11d48] rounded-lg text-[10px] font-black uppercase tracking-widest"><XCircle size={12} /> Rejected</span>;
            default:
                return <span className="flex items-center gap-1 px-2 py-1 bg-surface-secondary text-ink-secondary rounded-lg text-[10px] font-black uppercase tracking-widest">Draft</span>;
        }
    };
        ...
        <div className="space-y-8">
            <div className="flex justify-between items-center">
                ...
                <button
                    ...
                    className="px-6 py-3 bg-ink-primary text-white rounded-2xl font-black uppercase text-[10px] tracking-widest flex items-center gap-2 hover:scale-105 transition-all shadow-xl shadow-black/10"
                >
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    {promotions.map((promo) => (
                        <motion.div
                            ...
                            className="bg-surface-elevated rounded-[2.5rem] p-8 border border-border-primary shadow-sm hover:shadow-xl transition-all relative overflow-hidden group"
                        >
                            <div
                            ...
                            <div className="grid grid-cols-2 gap-4 mb-8">
                                <div className="p-4 bg-surface-secondary rounded-2xl">
                                    <p className="text-[9px] font-black text-ink-tertiary uppercase tracking-widest mb-1">Discount</p>
                                    <p className="text-xl font-black text-accent-400">{promo.discount_percent}% OFF</p>
                                </div>
                                <div className="p-4 bg-surface-secondary rounded-2xl">
                                    <p className="text-[9px] font-black text-ink-tertiary uppercase tracking-widest mb-1">Code</p>
                                    <p className="text-xl font-black text-ink-primary">{promo.promo_code || 'None'}</p>
                                </div>
                            </div>

                            <div className="flex items-center justify-between text-[10px] font-black uppercase tracking-widest text-ink-tertiary">
                                ...
                                {promo.rejection_reason && promo.approval_status === 'rejected' && (
                                    <div className="flex items-center gap-1 text-[#e11d48] cursor-help" title={promo.rejection_reason}>
                ...
                {isCreateModalOpen && (
                    <div className="fixed inset-0 z-200 flex items-center justify-center p-4 bg-ink-primary/60 backdrop-blur-sm">
                        <motion.div
                            ...
                            className="bg-surface-elevated w-full max-w-2xl rounded-[3rem] shadow-2xl overflow-hidden"
                        >
                            ...
                            <form onSubmit={handleCreate} className="p-10 space-y-6">
                                <div className="grid grid-cols-2 gap-6">
                                    <div className="space-y-2">
                                        <label className="text-[10px] font-black uppercase tracking-widest text-ink-tertiary">Promo Title</label>
                                        <input
                                            type="text"
                                            required
                                            ...
                                            className="w-full p-4 bg-surface-secondary rounded-2xl border-2 border-transparent focus:border-teal-500 outline-none transition-all font-bold"
                                        />
                                    </div>
                                    <div className="space-y-2">
                                        <label className="text-[10px] font-black uppercase tracking-widest text-ink-tertiary">Subtitle</label>
                                        <input
                                            type="text"
                                ...
                                <div className="space-y-2">
                                    <label className="text-[10px] font-black uppercase tracking-widest text-ink-tertiary">Description</label>
                                    <textarea
                                        value={formData.description}
                                        onChange={(e) => setFormData({ ...formData, description: e.target.value })}
                                        placeholder="Add more details about this offer..."
                                        className="w-full p-4 bg-surface-secondary rounded-2xl border-2 border-transparent focus:border-teal-500 outline-none transition-all font-bold min-h-[100px]"
                                    />
                                </div>

                                <div className="grid grid-cols-2 gap-6">
                                    <div className="space-y-2">
                                        <label className="text-[10px] font-black uppercase tracking-widest text-ink-tertiary">Discount %</label>
                                        <input
                                            type="number"
                                            required
                                    ...
                                    <div className="space-y-2">
                                        <label className="text-[10px] font-black uppercase tracking-widest text-ink-tertiary">Promo Code (Optional)</label>
                                        <input
                                            type="text"
                                            value={formData.promo_code}
                                            onChange={(e) => setFormData({ ...formData, promo_code: e.target.value.toUpperCase() })}
                                            placeholder="SUMMER20"
                                            className="w-full p-4 bg-surface-secondary rounded-2xl border-2 border-transparent focus:border-teal-500 outline-none transition-all font-bold uppercase"
                                        />
                                    </div>
                                </div>
                                ...
                                <button
                                    type="submit"
                                    className="w-full py-5 bg-ink-primary text-white rounded-2xl font-black uppercase text-[12px] tracking-widest shadow-2xl shadow-black/10 hover:scale-[1.02] active:scale-95 transition-all"
                                >
                                    Submit for Approval
                                </button>
                            </form>
