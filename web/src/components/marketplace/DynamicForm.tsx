    ...
    if (!schema) {
        return (
            <div className="p-8 bg-surface-secondary rounded-2xl border-2 border-dashed border-border-primary text-center">
                <p className="text-ink-tertiary font-bold">Standard details will be used for this category.</p>
            </div>
        );
    }
    ...
    const renderInnerField = (field: FormField, currentMetadata: any, onFieldChange: (name: string, value: any) => void) => {
        ...
        switch (field.type) {
                ...
                return (
                    <div className="space-y-6">
                        {items.map((item, index) => (
                            <div key={index} className="relative p-6 bg-surface-elevated border-2 border-border-primary rounded-[2rem] shadow-sm">
                                <button
                                    type="button"
                                    onClick={() => removeItem(index)}
                                    className="absolute -top-3 -right-3 w-8 h-8 bg-[#e11d48]/50 text-white rounded-full flex items-center justify-center shadow-lg hover:bg-[#e11d48] transition-colors z-10 font-bold"
                                >
                                    ×
                                </button>
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                    {field.schema?.map(subField => (
                                        <div key={subField.name} className={subField.type === 'textarea' || subField.type === 'repeatable_section' || subField.type === 'multiselect' ? 'md:col-span-2' : ''}>
                                            <label className="text-[10px] font-black uppercase tracking-widest text-ink-tertiary mb-2 block ml-2">
                                                {subField.label}
                                            </label>
                        ...
                        <button
                            type="button"
                            onClick={addItem}
                            className="w-full py-5 border-2 border-dashed border-teal-200 text-accent-400 rounded-[2rem] font-black uppercase tracking-widest hover:bg-accent-500/10/50 hover:border-teal-400 transition-all flex items-center justify-center gap-2 group"
                        >
            ...
            case 'variant_manager':
                ...
                return (
                    <div className="space-y-4">
                        {variants.map((v, i) => (
                            <div key={i} className="flex gap-4 items-start p-4 bg-surface-elevated rounded-2xl border border-border-primary">
                                <div className="flex-1">
                                    <label className="text-[10px] uppercase font-bold text-ink-tertiary">Option Name</label>
                                    <input
                                        placeholder="e.g. Size"
                                        value={v.name}
                                        onChange={e => updateVariantGroup(i, 'name', e.target.value)}
                                        className="w-full p-3 bg-surface-secondary rounded-xl font-bold text-ink-primary border-none focus:ring-2 focus:ring-accent-400"
                                    />
                                </div>
                                <div className="flex-[2]">
                                    <label className="text-[10px] uppercase font-bold text-ink-tertiary">Values (comma separated)</label>
                                    <input
                                ...
                                <button type="button" onClick={() => removeVariantGroup(i)} className="mt-6 text-[#fb7185] hover:text-[#e11d48] font-bold">×</button>
                            </div>
                        ))}
                        <button type="button" onClick={addVariantGroup} className="text-xs font-black uppercase text-accent-400 bg-accent-500/10 px-4 py-2 rounded-lg hover:bg-accent-500/15">+ Add Variant Option</button>
                    </div>
                );
            ...
            case 'calendar_builder':
                ...
                return (
                    <div className="bg-surface-elevated p-6 rounded-[2rem] border border-border-primary">
                        <div className="flex justify-between gap-2 mb-6">
                            {days.map(d => (
                                <button
                                    ...
                                    type="button"
                                    onClick={() => toggleDay(d)}
                                    className={`w-10 h-10 rounded-full font-bold text-xs flex items-center justify-center transition-all ${schedule.days?.includes(d) ? 'bg-accent-500 text-white shadow-lg shadow-accent-500/15' : 'bg-surface-secondary text-ink-tertiary'}`}
                                >
                                    {d.charAt(0)}
                                </button>
                            ))}
                        ...
                        <div className="flex gap-4">
                            <div className="flex-1">
                                <label className="text-[10px] uppercase font-bold text-ink-tertiary">Start Time</label>
                                <input
                                    type="time"
                                    value={schedule.start}
                                    onChange={e => onFieldChange(field.name, { ...schedule, start: e.target.value })}
                                    className="w-full p-3 bg-surface-secondary rounded-xl font-bold"
                                />
                            </div>
                            <div className="flex-1">
                                <label className="text-[10px] uppercase font-bold text-ink-tertiary">End Time</label>
                                <input
                                    type="time"
                                    value={schedule.end}
            ...
            case 'seasonal_rates':
                ...
                return (
                    <div className="space-y-3">
                        {rates.map((r, i) => (
                            <div key={i} className="p-4 bg-surface-elevated border border-border-primary rounded-2xl grid grid-cols-2 md:grid-cols-4 gap-3 items-end">
                                <div>
                                    <label className="text-[10px] uppercase font-bold text-ink-tertiary">Season Name</label>
                                    <input placeholder="e.g. Christmas" value={r.name} onChange={e => updateRate(i, 'name', e.target.value)} className="w-full p-2 bg-surface-secondary rounded-lg text-sm font-bold" />
                                </div>
                                <div>
                                    <label className="text-[10px] uppercase font-bold text-ink-tertiary">Price ($)</label>
                                    <input type="number" placeholder="0.00" value={r.price} onChange={e => updateRate(i, 'price', e.target.value)} className="w-full p-2 bg-surface-secondary rounded-lg text-sm font-bold" />
                                </div>
                                <div>
                                    <label className="text-[10px] uppercase font-bold text-ink-tertiary">Start Date</label>
                                    <input type="date" value={r.start} onChange={e => updateRate(i, 'start', e.target.value)} className="w-full p-2 bg-surface-secondary rounded-lg text-sm font-bold" />
                                </div>
                                <div className="flex gap-2">
                                    <div className="flex-1">
                                        <label className="text-[10px] uppercase font-bold text-ink-tertiary">End Date</label>
                                        <input type="date" value={r.end} onChange={e => updateRate(i, 'end', e.target.value)} className="w-full p-2 bg-surface-secondary rounded-lg text-sm font-bold" />
                                    </div>
                                    <button type="button" onClick={() => removeRate(i)} className="text-[#e11d48] font-bold p-2">×</button>
                                </div>
                            </div>
                        ))}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                {schema.fields.map((field) => (
                    <div key={field.name} className={`space-y-2 ${field.type === 'textarea' || field.type === 'multiselect' || field.type === 'repeatable_section' ? 'md:col-span-2' : ''}`}>
                        <label className="text-xs font-black uppercase tracking-[0.1em] text-ink-tertiary ml-2">
                            {field.label} {field.required && <span className="text-[#fb7185]">*</span>}
                        </label>
                        {renderInnerField(field, metadata, (name, val) => onChange({ ...metadata, [name]: val }))}
                        {field.helperText && <p className="text-xs text-ink-tertiary ml-2 font-medium">{field.helperText}</p>}
                    </div>
                ))}
            </div>
        </motion.div>
